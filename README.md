...
| `XDG_CACHE_HOME` | Override download cache directory (`~/.cache/ghdeb/`) |
| `XDG_CONFIG_HOME` | Override config directory (`~/.config/ghdeb/`) |
...
| Path | Purpose |
|------|---------|
| `/etc/ghdeb/catalog.toml` | System package catalog (auto-generated by `catalog init`) |
| `~/.cache/ghdeb/` | Downloaded .deb file cache |
| `/var/cache/ghdeb/installed.json` | Package state tracking |
